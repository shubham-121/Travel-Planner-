  ...
  return (
    <div className="border-custom min-h-[50%] max-h-[80vh] overflow-auto bg-white z-50 fixed p-6 rounded-lg shadow-xl w-full sm:w-[400px] md:w-[500px] lg:w-[600px] xl:w-[700px] left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
      ...
      <button
        ...
        onClick={() => dispatch(toggleWishModal())}
      >
        X
      </button>

      <div className="flex items-center justify-center flex-col">
        <p className="text-xl font-semibold mb-4">Your Wishlisted Places:</p>
        {wishlist.length > 0 ? (
          <RenderWishlist wish={wishlist} />
        ) : (
          <p>No wishlisted places</p>
        )}
      </div>
    </div>
  );
...
function RenderWishlist({ wish }) {
  const { wishlist } = useSelector((store) => store.Features); //global state
  const dispatch = useDispatch();
        ...
        {wish.map((wishPlace, idx) => (
          ...
          >
            ...
            <button
              ...
              onClick={() => dispatch(removeWishList(idx))} // Call the remove function
            >
              X
            </button>
          </li>
